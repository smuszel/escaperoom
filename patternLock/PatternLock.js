...
import { patternToWords, hashCode, gcd, prop } from './utils/libs.js';
import { registerEvent, getPixelRatio, raf } from './utils/dom.js';
    ...
    if (this._isDragging) {
      ...
      const lastNode = this.selectedNodes.reduce((prevNode, node) => {
        if (prevNode) {
          ...
          const p2 = {
            x: prevNode.row * this.interval.x,
            y: prevNode.col * this.interval.y,
          };
      ...
      if (lastNode && this.coordinates) {
        const prevPoint = {
          x: lastNode.row * this.interval.x,
          y: lastNode.col * this.interval.y,
        };
